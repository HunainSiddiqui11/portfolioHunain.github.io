...
export var SiteAssetsServerHttpHandlerBuilder = function(siteAssetsServerResponseBuilder, isBrowser, isStagingRequest) {
    if (isBrowser === void 0) {
        isBrowser = true;
    }
    ...
    var errorMessage = function(status, message) {
        return "server response: status: ".concat(status, ", message: ").concat(message);
    };
    ...
    var build = function(siteAssetsUrl, timeout, customCacheTTL, customRouting, bypassSsrInternalCache) {
        if (bypassSsrInternalCache === void 0) {
            bypassSsrInternalCache = false;
        }
        var headers = setHeaders(customCacheTTL, isBrowser, customRouting, isStagingRequest, bypassSsrInternalCache);
        return {
