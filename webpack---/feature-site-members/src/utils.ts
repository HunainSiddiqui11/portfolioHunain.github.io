...
export const memberDetailsFromIAMResponse = (response: IAMPlatformLoginResponse | LoginResponse): MemberDetails => {
	...
	return {
		...
		creationDate: identity!.createdDate! as string,
		lastUpdateDate: identity!.updatedDate! as string,
		emails: [],
		phones: [],
		addresses: [],
		labels: [],
...
export const toLoginResponse = (
	socialLoginResponse: IMemberPayload | IAMPlatformLoginResponse | LoginResponse
): { token?: string; member: MemberDetails; pages?: AuthorizedPages; status?: StateStatus } => {
	// `additionalData` property exist only in IAM response
	if (socialLoginResponse.additionalData) {
		...
		// @ts-expect-error in IAM response there is no smSession
		...
		// @ts-expect-error in IAM response there is no siteMemberDto
