...
import type { CommonProps } from './dialogService'
import { CAPTCHA_SETTINGS_OPTIONS } from './utils'
import { StateStatus, StatusV2, PrivacyStatus } from '@wix/ambassador-iam-authentication-v1-authentication/types'
...
export type IAMPlatformLoginResponse = {
	identity: Identity
	sessionToken: string
...
export interface ISiteMembersApi {
	...
	handleOauthToken(token: string, provider: string, mode: string, joinCommunityStatus: string): Promise<LoginResult>
...
export type SocialAuthComponentProps = {
	...
	// We are not passing the callbacks in DS
	// onTokenMessage about to be deprecated
	onTokenMessage?: (token: string, provider: string, joinCommunityStatus?: boolean) => Promise<LoginResult>
